...
function decide({body, association, login, pr, history, now}) {
  ...
  const field=name=>body.slice(match.index).match(new RegExp('^'+name+':\\s*(.+)$','mi'))?.[1]?.trim();
...
module.exports.run=async ({github,context,core})=>{
  ...
  if (!preflight.allow) {
    ...
    await core.summary.addHeading('Agent admission').addRaw(JSON.stringify({pr:number,head:pr.head.sha,base:pr.base.sha,...preflight})).write();
    if (!['no-explicit-review-command','untrusted-author'].includes(preflight.reason)) core.setFailed(`Review not started: ${preflight.reason}`);
    return;
  }
  ...
  if (!d.allow) core.setFailed(`Review blocked: ${d.reason}. No model started; unresolved findings still block release.`);
};
