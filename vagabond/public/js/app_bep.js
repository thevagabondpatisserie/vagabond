...
async function scrHome() {
  ...
  if (isSales()) {
    ...
    html += '<div class="sec">Bán hàng</div><div class="card">' +
      card('\ud83d\udcb5', 'Doanh thu Sales', 'Đơn online của sales: Pancake, Grab, Be', dsn, 'DS') +
      card('🧾', 'Doanh thu Cửa hàng', 'Bấm bill, thu tiền, ghi sổ tại quầy: District 1, NVHTN', 0, 'POS') + card('📑', 'Hợp đồng Event', 'Catering, teabreak, bánh thiết kế theo hợp đồng', 0, 'HDG') + '</div>';
  }
...
function posMoi() {
  posSepayNhan = 0;
  return { che_do: 'Tại chỗ', ma: '', bill: posMaBill(), pt: 'Tiền mặt', mtc: '', ten: '', sdt: '', giam: '', dua: '', ghi_chu: '', km: null, xhd_mo: false, xh: { mst: '', ten: '', dc: '', email: '' }, mon: [] };
}
...
async function scrPosChonQuay() {
  ...
  /* Thumbnail la anh cua hang that (anh Viet gui 09/08), nhin phat biet
     ...
     bieu tuong cu. */
  var html = '<div class="sec">Chọn quầy để tính tiền</div>';
  dsQ.forEach(function (q, i) {
    html += '<div class="card" data-q="' + i + '" style="margin-bottom:12px;overflow:hidden;cursor:pointer;padding:0">' +
      (q.anh
  ...
  html += '<div style="text-align:center;color:#a0a6b4;font-size:12px;padding:4px 10px 10px">Chọn đúng quầy mình đang đứng - doanh thu và đối soát tách theo từng quầy</div>';
  var b = frame('Doanh thu Cửa hàng', html);
  b.onclick = function (e) {
    var r = e.target.closest('[data-q]');
    if (!r) return;
    posQuay = dsQ[+r.getAttribute('data-q')];
    posHomNayTxt = null;
    go(scrPosQuay);
  };
}
...
async function scrPosQuay() {
  ...
  var giam = posSoTien(posDon.giam), dua = posSoTien(posDon.dua);
  ...
  html += '<div class="sec">Thanh toán</div><div class="card" style="padding:12px 14px;display:grid;gap:10px">' +
    ...
    '<span style="font-size:12.5px;color:#6b7280;font-weight:600">GIẢM GIÁ CẢ BILL (đ)</span>' +
    '<button id="posKm" style="border:1.5px solid #7fe5f6;background:#fff;color:#0b7c93;border-radius:8px;padding:5px 10px;font-size:12.5px;font-weight:700;cursor:pointer">🎟 Chọn voucher</button></div>' +
    (posDon.km
      ? '<div style="display:flex;align-items:center;gap:6px;background:#ecfeff;border:1.5px solid #7fe5f6;border-radius:9px;padding:8px 10px;margin-bottom:6px;font-size:13px">' +
        '<span style="flex:1">🎟 <b>' + h(posDon.km.ten) + '</b> · ' + (posDon.km.loai === 'Phần trăm' ? money(posDon.km.gia_tri) + '%' : money(posDon.km.gia_tri) + ' đ') + '</span>' +
        '<button id="posKmBo" style="border:0;background:transparent;color:#b3261e;font-size:16px;cursor:pointer">✕</button></div>'
      : '') +
    '<input class="tin" id="posGiam" placeholder="0" inputmode="numeric" value="' + (giam ? money(giam) : '') + '"' + (posDon.km ? ' readonly style="background:#f6f7f9"' : '') + '></div>' +
  ...
  html += '<div class="sec">Khách (không bắt buộc)</div><div class="card" style="padding:12px 14px;display:grid;gap:10px">' +
    '<input class="tin" id="posTen" placeholder="Tên khách" value="' + h(posDon.ten || '') + '">' +
  ...
  html += '<div class="card" style="padding:12px 14px;margin-top:10px">' +
    ...
    '<div style="flex:1"><b style="font-size:14px">Khách cần hoá đơn công ty?</b><div style="font-size:12px;color:#98a2b3">Không điền cũng được - khách quét QR cuối bill tự điền sau</div></div>' +
  ...
  html += '<div class="card" style="padding:12px 14px;display:grid;gap:6px;margin-top:10px">' +
    ...
    (giam ? '<div style="display:flex;justify-content:space-between;color:#b45309"><span>Giảm giá</span><span>&minus;' + money(giam) + ' đ</span></div>' : '') +
  ...
  var nKm = document.getElementById('posKm');
  if (nKm) nKm.onclick = async function () {
    posDoc();
    busy(true);
    var kq;
    try { kq = await api('vagabond.ban_hang.pos_ds_khuyen_mai', { quay: (posQuay && posQuay.ma) || '' }); }
    catch (e) { busy(false); return toast((e && e.message) || 'Không tải được danh mục khuyến mãi'); }
    busy(false);
    var ds = (kq && kq.km) || [];
    if (!ds.length) return toast('Chưa có chương trình khuyến mãi nào đang bật. Tạo trong Desk > Vagabond Khuyen Mai.', 4500);
    sheet('Chọn chương trình khuyến mãi', ds.map(function (k) {
      return { value: k.ten, label: k.ten, icon: '🎟', phu: (k.loai === 'Phần trăm' ? 'Giảm ' + money(k.gia_tri) + '% cả bill' : 'Giảm ' + money(k.gia_tri) + ' đ'), km: k };
    }), (posDon.km || {}).ten || '', function (o) {
      posDon.km = o.km;
      go(scrPosQuay, true);
    });
  };
  var nKmBo = document.getElementById('posKmBo');
  if (nKmBo) nKmBo.onclick = function () { posDoc(); posDon.km = null; posDon.giam = ''; go(scrPosQuay, true); };
...
function posKhoiQr(noiDung, tien) {
  ...
  if (posSepayNhan >= tien - 1) {
    return '<div style="border:2px solid #16a34a;border-radius:12px;padding:16px;text-align:center;background:#f0fdf4">' +
      ...
      '<div style="font-size:13px;color:#374151;margin-top:4px">SePay khớp nội dung <b>' + h(noiDung) + '</b>. Bấm Thu tiền để lưu bill rồi ghi sổ.</div>' +
      '</div>';
  }
...
function posQrSheet(soPhieu, tien, siName) {
  var q = (CFGBH || {}).qr_quay || {};
  ...
  ov.innerHTML = '<div class="shb" style="padding:20px 16px calc(env(safe-area-inset-bottom,0px) + 16px);text-align:center">' +
    ...
    (posBillVua ? '<button class="btn gh" data-in style="flex:0 0 34%;margin:0">🖨 In bill</button>' : '') +
    '<button class="btn" data-y style="flex:1;margin:0">Bill mới</button></div>' +
    (posBillVua && posCoNuoc(posBillVua.mon)
      ? '<div style="display:flex;gap:8px;margin-top:8px"><button class="btn gh" data-pm style="flex:1;margin:0">🧾 In phiếu làm món</button><button class="btn gh" data-tem style="flex:1;margin:0">🏷 In tem ly</button></div>'
      : '') +
    '</div>';
  document.body.appendChild(ov);
  ...
  var pid = setInterval(async function () {
    ...
    try {
      ...
      if (kq && kq.du) {
        ...
        if (ny && siName) { ny.textContent = '📒 Ghi sổ luôn - Bill mới'; ny.setAttribute('data-gs', '1'); }
      }
    } catch (e) { }
  }, 5000);
...
async function posLuuDon() {
  ...
  try {
    ...
    if (trung.length) canhBao += '\n⚠ CÓ ' + trung.length + ' BILL CÙNG SỐ TIỀN ' + money(phaiThu) + ' đ vừa lưu chưa đầy 2 phút. Có phải bấm trùng không? Kiểm trong danh sách bill trước khi thu tiếp.';
  } catch (e) { }
  ...
  posBillVua = {
    ...
    tong: tong, giam: giam, thu: thu, pt: laApp ? posDon.che_do : posDon.pt,
    ghi_chu: posDon.ghi_chu || '', ten: posDon.ten || '', tam_tinh: 0
  };
  posDon = posMoi();
  posHomNayTxt = null;
  /* Van la ma QR khach da quet luc nay, khong doi sang so phieu - de neu
     khach chua chuyen kip thi quet lai van ra dung noi dung. */
  if (laCK) return posQrSheet(maCk, thu, (r && r.name) || '');
  var ov = document.createElement('div'); ov.className = 'sh';
  ov.innerHTML = '<div class="shb" style="padding:22px 16px calc(env(safe-area-inset-bottom,0px) + 16px);text-align:center">' +
    ...
    '<div style="font-size:12.5px;color:#a0a6b4;margin-top:6px">' + h((r && r.name) || '') + ' · ghi sổ ngay tại quầy trong Bill hôm nay</div>' +
    '<div style="display:flex;gap:8px;margin-top:16px">' +
    '<button class="btn gh" data-in style="flex:1;margin:0">🖨 In bill</button>' +
    '<button class="btn" data-y style="flex:1;margin:0">🧾 Bill mới</button></div>' +
    (posCoNuoc(posBillVua.mon)
      ? '<div style="display:flex;gap:8px;margin-top:8px"><button class="btn gh" data-pm style="flex:1;margin:0">🧾 In phiếu làm món</button><button class="btn gh" data-tem style="flex:1;margin:0">🏷 In tem ly</button></div>'
      : '') +
    '<button class="btn gh" data-ds style="margin-top:8px">📋 Về danh sách bill</button></div>';
  document.body.appendChild(ov);
  ov.onclick = function (e) {
...
async function posInBill(d) {
  ...
  if (!d.tam_tinh && d.name && !d.xhd_url) {
...
async function posInTamTinh() {
  ...
  var giam = posSoTien(posDon.giam);
  var tong = posDon.mon.reduce(function (t, m) { return t + m.qty * m.rate; }, 0);
  var thu = Math.max(0, tong - giam);
  var ok = await confirmSheet('In phiếu tạm tính ' + money(thu) + ' đ',
    'Bill được lưu TẠM TÍNH - giữ món, chưa tính doanh thu.\nKhách thanh toán xong thì vào Bill hôm nay bấm Chốt.', 'Lưu và in phiếu');
  if (!ok) return;
  busy(true);
  var r;
  try {
  ...
  busy(false);
  posBillVua = { name: (r && r.name) || '', bill: posDon.bill, mon: posDon.mon.slice(), tong: tong, giam: giam, thu: thu, pt: '', ghi_chu: posDon.ghi_chu || '', ten: posDon.ten || '', tam_tinh: 1 };
  posInBill(posBillVua);
  posDon = posMoi();
  posHomNayTxt = null;
  toast('Đã lưu bill tạm tính ' + ((r && r.name) || ''));
  go(scrPosDs, true);
}
...
async function scrPosBill(name) {
  frame('Bill ' + name, '<div class="emp"><div class="e1">⏳</div></div>');
  var d;
  try { d = await api('frappe.client.get', { doctype: 'Sales Invoice', name: name }); }
  catch (e) { frame('Bill', '<div class="emp"><div class="e1">⚠️</div><div>' + h((e && e.message) || 'Không đọc được bill') + '</div></div>'); return; }
  await cfgBanHang();
  ...
  var html = '<div class="card" style="padding:12px 14px">' +
    '<div style="display:flex;justify-content:space-between"><b>' + h(maBill || d.name) + '</b><span style="color:#98a2b3;font-size:12px">' + h(d.name) + '</span></div>' +
    '<div style="font-size:12px;color:#6b7280">' + h(d.custom_nguon || '') + ' · ' + h(String(d.creation || '').slice(0, 16)) + '</div>' +
    '<div style="margin-top:4px">' + posChipBill({ docstatus: d.docstatus, vgb_tam_tinh: d.vgb_tam_tinh, vgb_pt_thanh_toan: d.vgb_pt_thanh_toan, sepay_du: 0, custom_hddt_so: d.custom_hddt_so, vgb_xhd_mst: d.vgb_xhd_mst, vgb_ghi_chu: '' }) + '</div></div>';
  html += '<div class="card" style="padding:6px 14px;margin-top:10px">';
  (d.items || []).forEach(function (m) {
    html += '<div style="display:flex;justify-content:space-between;padding:7px 0;border-bottom:1px solid #f0f2f6"><span>' + h(m.item_name || m.item_code) + ' <b>x' + money(m.qty) + '</b></span><b>' + money(m.amount) + '</b></div>';
  });
  if (d.discount_amount) html += '<div style="display:flex;justify-content:space-between;padding:7px 0;color:#b45309"><span>Giảm giá</span><b>-' + money(d.discount_amount) + '</b></div>';
  html += '<div style="display:flex;justify-content:space-between;padding:9px 0;font-size:16px"><b>' + (tamTinh ? 'TẠM TÍNH' : 'PHẢI THU') + '</b><b>' + money(d.grand_total) + ' đ</b></div></div>';
  if (nhap) {
    html += '<div class="sec">' + (tamTinh ? 'Khách thanh toán bằng gì?' : 'Phương thức thanh toán') + '</div>' +
      '<div class="card" style="padding:12px 14px;display:grid;gap:10px">' +
      '<div id="pbPt" style="display:grid;grid-template-columns:1fr 1fr;gap:8px">' + posNutPt(dsPt, d.vgb_pt_thanh_toan || '') + '</div>' +
      '<div id="pbQr"></div>' +
      '<input class="tin" id="pbMtc" placeholder="Mã tham chiếu (bill thẻ, mã giao dịch...)" value="' + h(d.vgb_ma_tham_chieu || '') + '">' +
      '<input class="tin" id="pbGhiChu" placeholder="Ghi chú bill" value="' + h(d.vgb_ghi_chu || '') + '"></div>';
  } else if (d.vgb_ghi_chu) {
    html += '<div class="card" style="padding:12px 14px;margin-top:10px;font-size:13.5px">📝 ' + h(d.vgb_ghi_chu) + '</div>';
  }
  var foot = '<div style="display:flex;gap:8px">' +
    '<button class="btn gh" id="pbIn" style="flex:0 0 30%;margin:0">🖨 In</button>' +
    (nhap
      ? (tamTinh
        ? '<button class="btn" id="pbChot" style="flex:1;margin:0">✅ Chốt bill - khách đã trả</button>'
    ...
    '</div>' +
    (posCoNuoc(d.items || [])
      ? '<div style="display:flex;gap:8px;margin-top:8px"><button class="btn gh" id="pbPhieuMon" style="flex:1;margin:0">🧾 In phiếu làm món</button><button class="btn gh" id="pbTemLy" style="flex:1;margin:0">🏷 In tem ly</button></div>'
      : '') +
    (nhap ? '<button class="btn gh" id="pbXoa" style="margin-top:8px;color:#b3261e;border-color:#fecaca">🗑 Xoá bill bấm nhầm</button>' : '');
  var b = frame('Bill ' + (maBill || d.name), html, { footer: foot });
  var PB_PT = d.vgb_pt_thanh_toan || '';
  function veQr() {
    var o = document.getElementById('pbQr');
    if (!o) return;
    o.innerHTML = PB_PT === 'Chuyển khoản' ? posKhoiQr(maBill || d.name, d.grand_total) : '';
  }
  ...
  async function luuVe(chot) {
    ...
    try {
      ...
      busy(false);
      toast(chot ? 'Đã ghi sổ ' + d.name : 'Đã chốt bill ' + d.name);
      posHomNayTxt = null;
      go(scrPosDs, true);
  ...
  if (nc) nc.onclick = async function () {
    var ok = await confirmSheet('Chốt bill ' + money(d.grand_total) + ' đ - ' + (PB_PT || 'chưa chọn'),
      'Khách đã thanh toán xong? Chốt xong bill thành doanh thu, ghi sổ được ngay.', 'Khách đã trả, chốt bill');
    if (ok) luuVe(false);
  };
  var ng = document.getElementById('pbGhiSo');
  if (ng) ng.onclick = async function () {
    var ok = await confirmSheet('Ghi sổ bill ' + money(d.grand_total) + ' đ',
  ...
  if (nx) nx.onclick = async function () {
    var ok = await confirmSheet('Xoá bill ' + (maBill || d.name) + '?', 'Chỉ xoá bill bấm nhầm. Xoá rồi không lấy lại được.', 'Xoá bill', true);
    if (!ok) return;
    busy(true);
    try { await api('vagabond.ban_hang.pos_xoa', { name: d.name }); busy(false); toast('Đã xoá'); posHomNayTxt = null; go(scrPosDs, true); }
    catch (e) { busy(false); toast((e && e.message) || 'Xoá lỗi', 4000); }
  };
  function pbBillObj() {
    return {
      ...
      mon: (d.items || []).map(function (m) {
        ...
        if (mo) tc = [mo[1]];
        return { item_code: m.item_code, ten: m.item_name || m.item_code, qty: m.qty, rate: m.rate, tc: tc };
      }),
  ...
  document.getElementById('pbIn').onclick = function () {
    /* posInBill tu xin link XHD khi thieu - in tu day hay in ngay sau khi
       thu tien deu co QR nhu nhau. In xong ve danh sach bill. */
    posInBill(pbBillObj());
    go(scrPosDs, true);
  };
...
function posInPhieuMon(d) {
  ...
  if (!nuoc.length) return toast('Bill không có món nước nào.');
  var w = window.open('', '_blank');
  if (!w) return toast('Trình duyệt chặn cửa sổ in. Cho phép popup rồi bấm lại.', 4000);
  var gio = new Date();
  var hs = function (n) { return (n < 10 ? '0' : '') + n; };
  ...
  w.document.write('<html><head><meta charset="utf-8"><title>Phiếu làm món ' + h(d.bill || d.name || '') + '</title><style>' +
    ...
    '</style></head><body>' +
    '<h1>PHIẾU LÀM MÓN</h1>' +
    '<div class="ph">' + h((posQuay && posQuay.ten) || '') + ' · bill <b>' + h(d.bill || d.name || '') + '</b> · ' + hs(gio.getHours()) + ':' + hs(gio.getMinutes()) + '</div>' +
    '<hr>' + rows +
    (d.ghi_chu ? '<div class="gc">Ghi chú: ' + h(d.ghi_chu) + '</div>' : '') +
...
function posInTemLy(d) {
  ...
  var tem = ly.map(function (m, i) {
    return '<div class="tem">' +
      ...
      '<div class="c">' + ((m.tc || []).length ? h(m.tc.join(', ')) : '100% đường · 100% đá') + '</div>' +
      '<div class="f"><span>' + h(d.bill || d.name || '') + '</span><span>Ly ' + (i + 1) + '/' + ly.length + '</span></div>' +
      '</div>';
  }).join('');
  w.document.write('<html><head><meta charset="utf-8"><title>Tem ly ' + h(d.bill || d.name || '') + '</title><style>' +
...
async function scrPosChotCa() {
  ...
  var html = '<div class="card" style="padding:12px 14px"><b>Ca ngày ' + h(k.ngay || today()) + ' · quầy ' + h(k.quay || '') + '</b>' +
    '<div style="font-size:12px;color:#98a2b3">' + (k.tong_bill || 0) + ' bill doanh thu · tổng ' + money(k.tong_tien || 0) + ' đ</div></div>';
  ...
  (k.pt || []).forEach(function (p) {
    ...
    html += dRow((laTm ? '💵 ' : '') + h(p.pt) + ' · ' + p.so + ' bill', p.tien,
      laTm ? 'PHẢI CÓ ĐỦ TRONG KÉT khi giao ca' : '', laTm ? '#b45309' : '');
  });
  if (!(k.pt || []).length) html += '<div style="padding:16px 0;color:#98a2b3;text-align:center">Chưa có bill doanh thu nào.</div>';
  html += '</div>';
  html += '<div class="sec">Đối soát chuyển khoản (SePay)</div><div class="card" style="padding:6px 14px">' +
    dRow('✅ SePay đã nhận', k.ck_ve || 0, 'khớp theo mã bill VGB trong nội dung CK', '#0f766e');
...
function posInChotCa(k) {
  ...
  var tt = k.tam_tinh || { so: 0, tien: 0 };
  var thanPt = (k.pt || []).map(function (p) { return dd(h(p.pt) + ' (' + p.so + ' bill)', money(p.tien) + ' đ'); }).join('');
  ...
  w.document.write('<html><head><meta charset="utf-8"><title>Chốt ca ' + h(k.quay || '') + '</title><style>' +
    ...
    '<hr><div class="s">TIỀN THEO PHƯƠNG THỨC</div>' + (thanPt || dd('Chưa có bill', '')) +
    '<hr><div class="s">ĐỐI SOÁT CHUYỂN KHOẢN</div>' +
    dd('SePay đã nhận', money(k.ck_ve || 0) + ' đ') + (thanThieu || dd('Không bill nào thiếu', '&#10003;')) +
    '<hr><div class="s">SỔ SÁCH</div>' +
    dd('Đã ghi sổ', (k.da_ghi || 0) + ' bill') + dd('Chưa ghi sổ', (k.chua_ghi || 0) + ' bill') +
    dd('Tạm tính còn treo', tt.so + ' bill · ' + money(tt.tien) + ' đ') +
