...
async function scrHome() {
  ...
  if (isSales()) {
    ...
    html += '<div class="sec">Bán hàng</div><div class="card">' +
      ...
      card('🧾', 'Doanh thu Cửa hàng', 'Bấm hoá đơn, thu tiền, ghi sổ tại quầy: District 1, NVHTN', 0, 'POS') + card('📑', 'Hợp đồng Event', 'Catering, teabreak, bánh thiết kế theo hợp đồng', 0, 'HDG') +
      card('🔐', 'Mã OTP quản lý', 'Cấp mã cho nhân viên sửa hoặc xoá hoá đơn', 0, 'OTP') + '</div>';
  }
...
function posMoi() {
  posSepayNhan = 0;
  return { che_do: 'Tại chỗ', ma: '', bill: posMaBill(), pt: 'Tiền mặt', mtc: '', ten: '', sdt: '', giam: '', dua: '', ghi_chu: '', km: null, so_ban: '', xhd_mo: false, xh: { mst: '', ten: '', dc: '', email: '' }, mon: [] };
}
...
async function scrPosQuay() {
  ...
  html += '<div class="sec">Thanh toán</div><div class="card" style="padding:12px 14px;display:grid;gap:10px">' +
    ...
    '<span style="font-size:12.5px;color:#6b7280;font-weight:600">GIẢM GIÁ CẢ HOÁ ĐƠN (đ)</span>' +
  ...
  if (nKm) nKm.onclick = async function () {
    ...
    sheet('Chọn chương trình khuyến mãi', ds.map(function (k) {
      return { value: k.ten, label: k.ten, icon: '🎟', phu: (k.loai === 'Phần trăm' ? 'Giảm ' + money(k.gia_tri) + '% cả hoá đơn' : 'Giảm ' + money(k.gia_tri) + ' đ'), km: k };
...
async function posLuuDon() {
  ...
  try {
    r = await api('vagabond.ban_hang.tao_don_tay', {
      ...
      giam_gia: giam, phi_ship: 0, quay: posQuay.ma || '', so_ban: posDon.so_ban || '',
...
async function posInBill(d) {
  ...
  if (!d.tam_tinh) {
    lien2 = '<div style="margin-top:4mm;padding-top:1mm;border-top:1px dashed #000;text-align:center;font-size:10px">✂ cắt tại đây</div>' +
      ...
      (d.so_ban ? '<div class="d"><span>Bàn: <b>' + h(d.so_ban) + '</b></span></div>' : '') +
...
async function posInTamTinh() {
  ...
  busy(false);
  posBillVua = { name: (r && r.name) || '', bill: posDon.bill, mon: posDon.mon.slice(), tong: tong, giam: giam, thu: thu, pt: '', ghi_chu: posDon.ghi_chu || '', ten: posDon.ten || '', so_ban: posDon.so_ban || '', tam_tinh: 1 };
...
async function scrPosBill(name) {
  ...
  if (suaMo) {
    ...
    foot = '<div style="display:flex;gap:8px">' +
      ...
      (nhap ? '<button class="btn gh" id="pbXoa" style="flex:1;margin:0;color:#b3261e;border-color:#fecaca">🗑 Xoá đơn</button>' : '') +
      '</div>';
  }
  ...
  /* ----- xoa: phai co OTP quan ly ----- */
  var nx = document.getElementById('pbXoa');
  if (nx) nx.onclick = async function () {
    var ok = await confirmSheet('Xoá đơn ' + (maBill || d.name) + '?',
      'Hoá đơn ' + money(d.grand_total) + ' đ sẽ bị xoá hẳn, không lấy lại được. Thao tác này cần mã OTP của quản lý và được ghi lại tên người xoá.', 'Xoá đơn', true);
    if (!ok) return;
    var otp = await posXinPhep('Xoá hoá đơn ' + (maBill || d.name));
    if (otp === null) return;
    busy(true);
    try { await api('vagabond.ban_hang.pos_xoa', { name: d.name, otp: otp }); busy(false); toast('Đã xoá hoá đơn ' + (maBill || d.name)); posHomNayTxt = null; go(scrPosDs, true); }
    catch (e) { busy(false); toast((e && e.message) || 'Xoá lỗi', 5000); }
  };
  ...
  if (ntm) ntm.onclick = async function () {
    ...
    posSheetMon(dsItemsCache.map(function (x) {
      ...
      if (!o.gia) return toast('Món ' + o.label + ' chưa có giá bán trong danh mục.', 4000);
      var vt = -1;
      posSua.mon.forEach(function (m, k) { if (m.item_code === o.value) vt = k; });
      if (vt >= 0) posSua.mon[vt].qty += 1;
      else posSua.mon.push({ item_code: o.value, ten: o.label, qty: 1, rate: o.gia, nhom: o.nhom, tc: [] });
      go(function () { scrPosBill(name); }, true);
    });
  };
  ...
  if (nls) nls.onclick = async function () {
    ...
    if (nhap) {
      goi.items = JSON.stringify(posSua.mon.map(function (m) {
        return { item_code: m.item_code, qty: m.qty, rate: m.rate, tuy_chon: (m.tc || []).join(', ') };
      }));
...
function posInTemLy(d) {
  ...
  var tem = ly.map(function (m, i) {
    return '<div class="tem">' +
      ...
      '<div class="f"><span>' + h(d.bill || d.name || '') + (d.so_ban ? ' · Bàn ' + h(d.so_ban) : '') + '</span><span>Ly ' + (i + 1) + '/' + ly.length + '</span></div>' +
      '</div>';
  }).join('');
