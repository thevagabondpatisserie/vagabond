...
(function () {
  ...
  var D = qs.get('d') || '', T = qs.get('t') || '', E = qs.get('e') || '';
  ...
  goi('xhd_khach_xem', { d: D, t: T, e: E }).then(function (b) {
    if (b.da_xuat) {
      goc.innerHTML = '<div class="ok"><div class="e">✅</div><b>Bill này đã gửi hoặc đang chờ đối chiếu hoá đơn điện tử.</b><div class="phu" style="margin-top:6px">Cần điều chỉnh thông tin thì anh chị liên hệ tiệm giúp em.</div></div>';
      return;
    }
    var ngay = String(b.posting_date || '').split('-').reverse().join('/');
    goc.innerHTML =
      '<h1>Thông tin xuất hoá đơn</h1>' +
      '<div class="phu">Anh chị điền thông tin trong thời hạn của link.<br>Hoá đơn điện tử sẽ gửi về email trong ngày.</div>' +
      ...
      '<div class="card">' +
      '<label>Mã số thuế (10, 12 hoặc 13 số)</label><input id="xMst" inputmode="numeric" value="' + h(b.vgb_xhd_mst) + '" placeholder="Nhập xong máy tự tra tên công ty">' +
    ...
    oL.onclick = function () {
      ...
      goi('xhd_khach_luu', { d: D, t: T, e: E, mst: so, ten: oT.value.trim(), dia_chi: oD.value.trim(), email: oE.value.trim() })
        .then(function () {
