...
@ca("đuôi số nhận rác thì NÉM LỖI, không bao giờ trả ra một mã trông như thật")
def _duoi_rac():
	# Ban dau tra "000000" cho rac. Codex bac: mot lan cap so hong thanh mot
	# ma trong nhu that, hai lan hong la hai chung tu cung mang 000000.
	for rac in (None, "bay nhieu", -5, 0):
		try:
			mps.duoi_so(rac)
			dung("phải ném lỗi với %r" % (rac,), False)
		except mps.LoiCapSo:
			pass
	m = _py("ma_phieu_sx.py")
	dung("không còn chỗ nào đổi rác thành 0", "so = 0\n" not in m.split("def duoi_so(")[1].split("\ndef ")[0])
...
@ca("cấp số hỏng thì hook CHẶN với câu rõ, không lùi về naming_series")
def _hook_lenh_hong():
	# Ban dau hook nuot loi va de trong ten cho Frappe dat theo chuoi thang.
	# Codex bac tren #215: mot ma kieu cu chen vao giua day moi la trai yeu
	# cau tang lien tuc, ma nguoi tao phieu khong he biet.
	def no(khoa):
	...
	try:
		mps.dat_ten_lenh(d)
		_tra_lai()
		dung("phải ném lỗi ra cho người tạo phiếu thấy", False)
	except Exception as e:
		_tra_lai()
		dung("câu nói rõ là cấp mã hỏng", "Không cấp được mã lệnh" in str(e))
		dung("nói dữ liệu chưa mất, bấm lại", "chưa mất" in str(e))
		la("KHÔNG để lại tên nào", d.name, None)


@ca("dòng đếm chưa được dựng thì _so_moi CHẶN, không để getseries tự tạo ở 1")
def _chua_dung_dong_dem():
	that = mps._dong_dem_dang_co
	try:
		mps._dong_dem_dang_co = lambda khoa: None
		try:
			mps._so_moi("LSX")
			dung("phải chặn", False)
		except mps.LoiCapSo as e:
			dung("bảo chạy lại migrate", "migrate" in str(e))
	finally:
		mps._dong_dem_dang_co = that
...
@ca("đặt tên lô hỏng cũng CHẶN, không để lô mang số kiểu cũ chen vào dãy mới")
def _hook_lo_hong():
	def no(khoa):
		raise RuntimeError("gia lap tabSeries hong")
	...
	try:
		...
		_tra_lai()
		dung("câu nói rõ là cấp mã lô hỏng", "Không cấp được mã lô" in str(e))


@ca("năm số liên tiếp đều có người mang thì chặn, không lặng lẽ bỏ qua")
def _nam_lan_trung():
	mps._so_moi = lambda khoa: "000057"
	mps._chua_ai_mang = lambda dt, ten: False
	mps._hom_nay = lambda: "2026-09-06"
	d = _Ho("Work Order")
	try:
		mps.dat_ten_lenh(d)
		...
		_tra_lai()
		dung("nói bộ đếm lệch", "lệch" in str(e))


# ------------------------------------------------------------ dòng đếm


@ca("đuôi đã cấp đọc ra từ mọi kiểu mã cũ, không có đuôi thì None")
def _duoi_da_cap():
	la("kiểu ngày", mps.duoi_so_cua("LSX-060926-000057"), 57)
	la("kiểu tháng", mps.duoi_so_cua("LSX-26-09-0010"), 10)
	la("kiểu năm", mps.duoi_so_cua("LSX-2026-00113"), 113)
	la("mã ERPNext gốc", mps.duoi_so_cua("MFG-WO-2026-00048"), 48)
	la("lô kiểu cũ", mps.duoi_so_cua("LO-260906-0028"), 28)
	la("không có đuôi", mps.duoi_so_cua("KKK2600007-NVLT00026"), None)
	la("rỗng", mps.duoi_so_cua(""), None)


@ca("mốc bảo toàn là ĐUÔI LỚN NHẤT đã cấp, không phải số chứng từ")
def _moc_bao_toan():
	# Codex: 56 lenh nhung co lenh mang duoi 113. Dung o 56 la lenh moi mang
	# so nho hon lenh cu.
	ten = ["MFG-WO-2026-00048", "LSX-2026-00113", "LSX-26-09-0010"]
	la("ba lệnh, đuôi lớn nhất 113", mps.moc_bao_toan(ten, []), 113)
	la("dòng đếm cùng họ cao hơn thì lấy dòng đếm", mps.moc_bao_toan(ten, [200, "5"]), 200)
	la("không có gì thì 0", mps.moc_bao_toan([], []), 0)
	la("bỏ qua rác trong dòng đếm", mps.moc_bao_toan(ten, [None, "x"]), 113)


class _DbGia(object):
	"""Bảng tabSeries giả cho `dat_moc_bo_dem`."""

	def __init__(self, dong, hong=False):
		self.dong = dict(dong)
		self.hong = hong
		self.ghi = []

	def sql(self, cau, ts=None):
		if self.hong:
			raise RuntimeError("gia lap DB hong")
		c = " ".join(cau.split()).lower()
		if c.startswith("select") and "like" in c:
			tt = ts[0].rstrip("%")
			return [(v,) for k, v in self.dong.items() if k.startswith(tt)]
		if c.startswith("select"):
			return [(self.dong[ts[0]],)] if ts[0] in self.dong else []
		if c.startswith("insert"):
			self.dong[ts[0]] = ts[1]; self.ghi.append(("insert", ts[0], ts[1])); return []
		if c.startswith("update"):
			if self.dong.get(ts[1], 0) < ts[2]:
				self.dong[ts[1]] = ts[0]; self.ghi.append(("update", ts[1], ts[0]))
			return []
		raise RuntimeError("cau la: " + cau)


def _voi_db(db, ten_theo_khoa):
	that = (mps.frappe.db, mps.frappe.get_all)
	mps.frappe.db = db
	mps.frappe.get_all = lambda dt, **k: list(ten_theo_khoa.get(dt, []))
	return that


def _tra_db(that):
	mps.frappe.db, mps.frappe.get_all = that


@ca("dựng dòng đếm: chưa có thì dựng ở mốc bảo toàn, không phải ở số chứng từ")
def _dung_o_moc():
	db = _DbGia({})
	that = _voi_db(db, {"Work Order": ["MFG-WO-2026-00048", "LSX-2026-00113"],
		"Batch": ["LO-260905-0014", "LO-260906-0028"]})
	try:
		ra = mps.dat_moc_bo_dem()
	finally:
		_tra_db(that)
	la("LSX dựng ở 113", db.dong.get("LSX"), 113)
	la("LO dựng ở 28", db.dong.get("LO"), 28)
	dung("báo đúng việc", "113" in ra["LSX"] and "28" in ra["LO"])


@ca("dựng dòng đếm: đã có mà THẤP hơn mốc thì nâng lên, cao hơn thì giữ, không bao giờ hạ")
def _khong_ha():
	db = _DbGia({"LSX": 5, "LO": 999})
	that = _voi_db(db, {"Work Order": ["LSX-2026-00113"], "Batch": ["LO-260906-0028"]})
	try:
		ra = mps.dat_moc_bo_dem()
	finally:
		_tra_db(that)
	la("LSX 5 nâng lên 113", db.dong["LSX"], 113)
	la("LO 999 giữ nguyên, không hạ về 28", db.dong["LO"], 999)
	dung("không có lệnh hạ nào", all(g[0] != "update" or g[2] >= 113 for g in db.ghi))


@ca("dựng dòng đếm: đọc mốc HỎNG thì không dựng gì, không mặc định 0")
def _doc_moc_hong():
	# Dung bua o 0 la lenh moi mang so nho hon lenh cu. Codex chot: loi doc
	# moc khong duoc mac dinh 0 roi tu dung.
	db = _DbGia({}, hong=True)
	that = _voi_db(db, {"Work Order": ["LSX-2026-00113"]})
	try:
		ra = mps.dat_moc_bo_dem()
	finally:
		_tra_db(that)
	la("không dựng dòng nào", db.dong, {})
	dung("báo lỗi rõ", "loi" in ra["LSX"] and "loi" in ra["LO"])
	m = _py("ma_phieu_sx.py")
	doan = m.split("def dat_moc_bo_dem(")[1].split("\ndef ")[0]
	dung("không có lệnh xoá", "delete" not in doan.lower())
	dung("không còn đếm bản ghi làm mốc", "db.count" not in doan)
...
def _cap_so_nguyen_tu():
	...
	dung("chặn khi dòng đếm chưa có", "_dong_dem_dang_co(khoa) is None" in doan)
