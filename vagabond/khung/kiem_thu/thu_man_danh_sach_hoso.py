...
def _bo_ca_hanh_vi():
	...
	# Codex neu vong bon, ba diem, deu dung:
	hv = io.open(os.path.join(GOI, "khung", "kiem_thu", "hanh_vi", "chay.js"),
		encoding="utf-8").read()
	# 1. Ten toan cuc chua dat phai NEM LOI chu khong duoc nuot thanh ham rong,
	#    khong thi go sai ten ham van bao xanh.
	dung("tên toàn cục lạ thì ném lỗi", "throw new ReferenceError(" in hv)
	dung("có danh sách tên được phép giả, viết ra rõ ràng", "var CHO_GIA = [" in hv)
	# 2. Hop chon phai la ham THAT, khong duoc thay bang stub.
	dung("nạp hoiChon thật", "layHam(hop, 'hoiChon')" in hv)
	dung("nạp cả bộ ô tìm thật của hộp chọn", "layHam(hop, 'vgbNoiOTim')" in hv)
	la("không còn stub hoiChon", "hoiChon: function (" in hv, False)
	# 3. Ca treo phai thanh do. Khong co han gio thi tien trinh thoat lang le
	#    voi ma 0, tuc la hong ma bao xanh.
	dung("mỗi ca có hạn giờ", "HAN_GIO_MS" in hv)
	dung("quá hạn thì ném lỗi", "Ca treo qua" in hv)
...
def _dang_ky():
	...
	# So doi tu 434 sang 436 ngay 06/09/2026: PR #208 gop vao main truoc, no
	# lay 435, va so phien ban CHI DUOC TANG (AGENTS.md dieu 4). Dong #v435 cua
	# ho GIU NGUYEN, dong cua minh doi so roi dat sau.
	dung("có dòng v436", "vagabond.patches.dong_bo_cau_truc #v436" in dong)
	for cu in ("#v431", "#v432", "#v433", "#v435"):
