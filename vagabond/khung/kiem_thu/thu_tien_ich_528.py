...
class _San:
	...
	def __init__(self, bts, mau=None, dung_roi=(), ma_gd="", ncc=NCC, chen=None):
		self.chen = chen
		self.cau_khoa = []
		...
		self.f = NS(get_all=self.get_all, get_doc=self.get_doc, get_meta=lambda dt: NS(has_field=lambda o: o == dca.O_MAU),
			...
			db=NS(get_value=self.get_value, exists=lambda dt, ten: ten in self.bts, set_value=self.set_value, sql=self.sql))

	def sql(self, cau, ts=None, as_dict=False, **k):
		"""Câu SQL chạy THẬT trên sqlite dựng từ bảng NCC giả (điều 17a).
		chen() mô phỏng lượt khác commit xong trong lúc lượt này chờ khoá."""
		import re
		import sqlite3
		if self.chen:
			self.chen(self)
		self.cau_khoa.append("for update" in cau)
		con = sqlite3.connect(":memory:")
		con.row_factory = sqlite3.Row
		con.execute("create table `tabSupplier` (name, `%s`)" % self.dca.O_MAU)
		for n, m in self.mau.items():
			con.execute("insert into `tabSupplier` values (?, ?)", (n, m))
		cur = con.execute(re.sub(r"\s+for update\s*$", "", cau.strip()).replace("%s", "?"), tuple(ts or ()))
		return [_D({c: x[c] for c in x.keys()}) for x in cur]
...
# ---------------------------------------------- Codex #370 vòng 1 trên 5c98444


@ca("v528 Codex #370 K1: dòng đúng mẫu, đúng tiền mà mang mã APP của hồ sơ KHÁC thì không tự khớp, không gợi ý")
def _k1_ma_khac():
	khac = _bt("ACC-BTN-2026-06999", "2026-09-23", "WATER BT WATER APP.26.09.999 x.BP1", ref="FT9")
	s = _San([khac], mau={NCC: "WATER BT WATER"})
	la("không tự khớp theo mẫu", s.chay(lambda d: d.chon(s.doc)), None)
	s = _San([khac, G_BT], mau={NCC: "WATER BT WATER"})
	la("dòng không mã vẫn khớp, dòng mã khác bị bỏ", getattr(s.chay(lambda d: d.chon(s.doc)), "name", None), "ACC-BTN-2026-06151")
	s = _San([khac, G_BT])
	la("gợi ý bỏ dòng mã khác", [r["name"] for r in s.chay(lambda d: d.goi_y_khong_ma(s.doc))], ["ACC-BTN-2026-06151"])
	khac_ref = _bt("ACC-BTN-2026-07001", "2026-09-23", MO_TA_BT, ref="APP2609999")
	s = _San([khac_ref], mau={NCC: "WATER BT WATER"})
	la("mã khác nằm ở ô tham chiếu cũng bỏ", s.chay(lambda d: d.chon(s.doc)), None)
	from vagabond.khop_sao_ke import co_ma_app_khac
	la("mã của chính hồ sơ không bị coi là mã khác", co_ma_app_khac("WATER BT WATER APP.26.09.100", APP), False)


@ca("v528 Codex #370 K3: nhớ mẫu kiểm lại chủ mẫu và đọc lại ô NCC dưới khoá; lượt khác vừa commit thì không ghi đè")
def _k3_khoa():
	def ncc_khac_vua_nho(san):
		san.mau["CAP-NUOC-TH"] = "WATER BT WATER"
	s = _San([G_BT], ma_gd="ACC-BTN-2026-06151", chen=ncc_khac_vua_nho)
	kq = s.chay(lambda d: d.nho_mau(APP, "WATER BT WATER"))
	dung("NCC khác vừa nhớ cùng mẫu: chặn, nêu tên NCC kia", "CAP-NUOC-TH" in kq.get("loi", ""))
	la("không ghi vào NCC này", s.mau.get(NCC), None)
	la("câu đọc là current read có khoá", s.cau_khoa, [True])

	def cung_ncc_vua_them(san):
		san.mau[NCC] = "EVN-HCM ELECTRIC"
