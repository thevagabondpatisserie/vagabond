...
async function canh(ngay,canhBao,cu=false){
 ...
 const c={document,console,Date,Math,JSON,Promise,setTimeout,
  ...
  getList:async(dt)=>dt==='Item'?[{name:'M',has_batch_no:1,shelf_life_in_days:90}]:[],
  ...
  api:async(url,args)=>{
   goi.push({url,args});
   if(url==='frappe.client.get')return {doctype:'Purchase Receipt',name:'PR-1',modified:'2026-09-13',items:[{name:'D1',item_code:'M',qty:1,rate:1000,warehouse:'K'}]};
...
(async()=>{await canh('',['Chưa có hạn sử dụng: Món']);await canh('2026-09-01',['Hạn dùng cần kiểm tra: Món']);await canh('',['Chưa có hạn sử dụng: Món'],true);await canh('2026-09-01',['Hạn dùng cần kiểm tra: Món'],true);console.log('PASS 489: shelf90, clear date, min0 expired, payload and warning before back');})().catch(e=>{console.error(e);process.exit(1);});
