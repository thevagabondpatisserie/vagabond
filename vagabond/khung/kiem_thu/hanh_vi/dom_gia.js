...
ElementGia.prototype.closest = function (chon) {
  var t = chonThuocTinh(chon);
  var nut = this;
  while (nut) {
    if (t && nut.hasAttribute && nut.hasAttribute(t)) return nut;
    nut = nut.parentNode;
  }
  return null;
};
...
ElementGia.prototype.querySelectorAll = function (chon) {
  var t = chonThuocTinh(chon);
  if (!t) throw new Error('DOM gia chi hieu bo chon dang [thuoc-tinh]: ' + chon);
  var ra = [];
  di(this, function (n) { if (n.hasAttribute(t)) ra.push(n); });
  return ra;
};
