# -*- coding: utf-8 -*-
"""v505: man Xuat kho phuc vu ban hang.
...
@ca("v505: so da dung la ton tren so tru con lai thuc te")
def _phep_tinh():
...
@ca("v505: chua dem thi bo qua chu khong phai loi")
def _chua_dem():
...
@ca("v505: dem it hon so thi ghi vao phieu")
def _co_hang_di_ra():
...
@ca("v505: dem NHIEU hon so thi khong nhan, va chi duong sang Kiem ke")
def _dem_nhieu_hon():
...
@ca("v505: banh thanh pham khong di qua man nay, va may noi ro vi sao")
...
@ca("v505: dung nam nhom trong pham vi, moi nhom mot nhan tai khoan")
def _pham_vi():
...
@ca("v505: dong mang nhom ngoai pham vi thi bi chan ngay o phep soat")
...
@ca("v505: soat HET moi dong roi moi tra loi, khong dung o dong hong dau")
def _soat_het():
...
@ca("v505: bang chua dem dong nao thi noi ro, khong de phieu rong")
def _bang_rong():
	se_ghi, nhac = xp.soat_bang_dem([
...
@ca("v505: doan bo phan chiu chi phi tu ten kho")
def _goi_y_bo_phan():
...
@ca("v505: dien giai doc duoc ngay tren ban may tinh")
def _dien_giai():
...
def _chay_luu(dong, bp="Cửa hàng D1"):
	"""Chay chinh `xp.luu`, chi chan phan cham co so du lieu."""
	giu = {}

	def _get_all(dt, **k):
		if dt == "Account":
			return _tai_khoan_gia()
		if dt == "Item":
...
@ca("v505: luu that - moi nhom mon ve dung tai khoan cua no")
...
@ca("v505: luu that - nhom mon doc tu danh muc, KHONG tin app gui len")
def _khong_tin_app():
...
@ca("v505: luu that - dong chua dem khong vao phieu")
...
@ca("v505: luu that - chua chon bo phan thi khong luu")
def _thieu_bo_phan():
	loi = ""
	try:
...
@ca("v505: hai man cung ghi vao mot o, moi man phai loc DUNG ma cua minh")
...
@ca("v505: ghi so phieu cua man kia thi bi chan o ca hai chieu")
def _ghi_so_cheo():
