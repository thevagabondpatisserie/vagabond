...
def thu_giu_noi_dung():
    nd = copy.deepcopy(MAC_DINH)
    nd["khoi"].reverse()
    nd["khoi"][0]["hien"] = False
    ra = chuan_hoa(nd)
    la("Giữ nguyên nội dung", ra, nd)
    ra["khoi"][0]["tieu_de"] = "Đã sửa"
    dung("Không sửa dữ liệu gốc", nd["khoi"][0]["tieu_de"] != "Đã sửa")
...
def thu_gioi_han():
    ...
    for truong, v in (("hien", "false"), ("noi_dung", "a" * 4001), ("id", "<script>"), ("loai", "html"), ("anh", 123)):
        nd = copy.deepcopy(MAC_DINH); nd["khoi"][0][truong] = v
