...
const r=spawnSync(process.execPath,[path.join(__dirname,'kiem_pancake.cjs')],{
  env:{...process.env,VGB_ARTIFACTS:out,VGB_KIEM_PAGEERROR:'1'},encoding:'utf8',timeout:180000
});
if(r.error||r.signal)throw Error('Tiến trình kiểm Desk lỗi/hết giờ: '+(r.error?.message||r.signal));
...
if(!evidence.errors.some(m=>m.includes('THU210_PAGEERROR')))throw Error('Không có pageerror thật');
