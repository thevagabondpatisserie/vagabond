...
def tao():
    ...
    roles = json.loads((goc / 'vai-fixture.json').read_text())
    users = [roles[k]['user'] for k in ('sales', 'bep', 'ke_toan')]
    names = {u: frappe.db.get_value('User', u, 'full_name') for u in users}
    assert len(set(users)) == 3 and all(names.values())
    expected = {}
    ds = []
    for i in range(300):
        ma = 'DO257-HS-%04d' % i
        d = frappe.get_doc({'doctype': 'Vagabond Ho So TT', 'ma': ma,
            ...
            'nguoi_tao': users[i % len(users)], 'ghi_chu': 'Tải thử ' + ma,
        ...
        assert d.nguoi_tao == users[i % len(users)]
        expected[d.name] = {'user': d.nguoi_tao, 'name': names[d.nguoi_tao]}
        ds.append(d.name)
    frappe.db.commit()
    assert frappe.db.count('Vagabond Ho So TT', {'name': ['in', ds]}) == 300
    (goc / 'tai-ho-so.json').write_text(json.dumps({'ten': ds, 'so_ho_so': 300,
        'nguoi_tao': expected, 'tu_khoa': 'DO257-HS-', 'tong_tien': 1800000, 'tim_ma': ds[-1]}))
