...
def bang_chung(name):
    ...
    for loai in ('ncc','ke-toan'):
        rows=frappe.get_all('Email Queue',filters={'reference_doctype':'Vagabond Ho So TT','reference_name':name,'message_id':ma_thu(name,loai)},fields=['name','status','creation','show_as_cc'],limit_page_length=0)
        for r in rows:
            ds=frappe.get_all('Email Queue Recipient',filters={'parent':r.name},fields=['recipient','status'],order_by='idx',limit_page_length=0)
            ra.append({'loai':loai,'ma':r.name,'trang_thai':r.status,'luc':str(r.creation),'cc':r.show_as_cc or '', 'nguoi_nhan':ds})
    return ra
