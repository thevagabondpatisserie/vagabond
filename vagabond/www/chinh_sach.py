...
/giao-hang-doi-tra. Ba đường cùng về trang này qua website_route_rules, khoá
trang đi theo `defaults`.
...
def get_context(context):
	...
	context.no_cache = 1
	khoa = str(frappe.form_dict.get("khoa") or "")
	ngon_ngu = "en" if str(frappe.form_dict.get("ngon_ngu") or "") == "en" else "vn"
	trang = noi_dung_web.trang_chinh_sach(khoa, ngon_ngu)
	if not trang:
